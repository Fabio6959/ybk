...
def rechunk_recompress_array(group, name, chunks=None, chunk_length=None, compressor=None, tmp_key="_temp"):
    ...
    if (chunks == old_arr.chunks):
        # no change
        return old_arr

    # rechunk recompress
    # zarr 3.x doesn't support group.move(), so we use a different approach
    # Read the entire array into memory and create a new one with the desired chunks
    data = old_arr[:]
    
    # Delete the old array
    del group[name]
    
    # Create a new array with the desired chunks
    new_arr = group.zeros(
        name=name,
        shape=old_arr.shape,
        dtype=old_arr.dtype,
...
class ReplayBuffer:
    ...
    def __init__(self, root: Union[zarr.Group, Dict[str, dict]]):
        ...
        if episode_ends.size > 0:
            for key in root["data"].keys():
                value = root["data"][key]
                assert (
                    value.shape[0] == episode_ends[-1]
                    ), f"{key} {value.shape} {episode_ends.shape} "
        self.root = root
    ...
    def create_empty_zarr(cls, storage=None, root=None):
        ...
        if "episode_ends" not in meta:
            episode_ends = meta.zeros(name="episode_ends", shape=(0,), dtype=np.int64, overwrite=False)
    ...
    def create_from_path(cls, zarr_path, mode="r", **kwargs):
        ...
        Slower.
        """
        store = zarr.storage.LocalStore(os.path.expanduser(zarr_path))
        group = zarr.open(store=store, mode=mode)
            ...
            for key in keys:
                ...
                if cks == value.chunks:
                    ...
                    n_copied, n_skipped, n_bytes_copied = zarr.copy(
                        source=value, dest=data_group, name=key, chunks=cks, if_exists=if_exists
                    )
        ...
        **kwargs,
        ...
        for key, value in self.root["data"].items():
            ...
            if isinstance(value, zarr.Array):
                ...
                # numpy
                _ = data_group.array(name=key, data=value, chunks=cks)
        return store
    ...
    def _resolve_array_compressor(cls, compressors: Union[dict, str, numcodecs.abc.Codec], key, array):
        ...
        if isinstance(compressors, dict):
            if key in compressors:
                ...
                # zarr 3.x doesn't have compressor attribute, use None
                cpr = None
        else:
    ...
    @property
    def n_steps(self):
        if self.episode_ends.size == 0:
            return 0
        return int(self.episode_ends[-1])

    @property
    def n_episodes(self):
        return self.episode_ends.size
    ...
    def add_episode(
        ...
        new_len = int(curr_len) + int(episode_length)
        ...
        for key, value in data.items():
            ...
            if key not in self.data:
                if is_zarr:
                    cks = self._resolve_array_chunks(chunks=chunks, key=key, array=value)
                    cpr = self._resolve_array_compressor(compressors=compressors, key=key, array=value)
                    arr = self.data.zeros(name=key, shape=new_shape, chunks=cks, dtype=value.dtype)
                else:
    ...
    def get_episode(self, idx, copy=False):
        idx = list(range(self.episode_ends.size))[idx]
        start_idx = 0
        if idx > 0:
    ...
    def set_compressors(self, compressors: dict):
        ...
        for key, value in compressors.items():
            if key in self.data:
                arr = self.data[key]
                # zarr 3.x doesn't support compressor setting, skip
                pass
