...
sys.path.insert(-1, f"{os.path.dirname(os.path.abspath(__file__))}/../")
...
def _ml1_env_names():
    tasks = list(_env_dict.ML1_V3["train"])
    assert len(tasks) == 50
    return tasks
...
class ML1(Benchmark):
    ...
    def __init__(self, env_name, seed=None):
        super().__init__()
        if not env_name in _env_dict.ALL_V3_ENVIRONMENTS:
...
class ML45(Benchmark):
    def __init__(self, seed=None):
        ...
        train_kwargs = _env_dict.ML45_ARGS_KWARGS
        self._train_tasks = _make_tasks(self._train_classes, train_kwargs, _ML_OVERRIDE, seed=seed)
        test_kwargs = _env_dict.ML45_ARGS_KWARGS
        self._test_tasks = _make_tasks(self._test_classes, test_kwargs, _ML_OVERRIDE, seed=seed)
